...
def rmetrics(a,b):
    ...
    #ssim
    ssim = compare_ssim(a, b, channel_axis=2, data_range=255)
...
class Visualizer():
    ...
    def print_current_val_mtx(self, epoch, t_comp, t_data, mtpfm):
        ...
        print(message_pfm, flush=True)  # print the message
        with open(self.log_pfm_val_name, "a") as log_pfm_val_file:
            log_pfm_val_file.write('%s\n' % message_pfm)  # save the message
            log_pfm_val_file.flush()

    def cal_current_pfm(self, epoch, visuals):
        if 'fake_B' in visuals and 'real_B' in visuals:
            im_out = util.tensor2im(visuals['fake_B'])
            im_GT = util.tensor2im(visuals['real_B'])
        elif 'fake' in visuals and 'real' in visuals:
            im_out = util.tensor2im(visuals['fake'])
            im_GT = util.tensor2im(visuals['real'])
        else:
            images = [util.tensor2im(v) for v in visuals.values()]
            im_out = images[1] if len(images) > 1 else images[0]
            im_GT = images[2] if len(images) > 2 else images[-1]
